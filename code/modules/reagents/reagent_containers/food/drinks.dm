...
/obj/item/weapon/storage/pickled_cucumbers
	name = "Jar of Pickled Cucumbers"
	desc = "Don't mix with milk."
	icon = 'icons/obj/drinks.dmi'
	icon_state = "empty_jar"
	var/cucumbers = 0
	var/list/list_reagents = list("brine" = 90)
	var/volume = 90
...
/obj/item/weapon/storage/pickled_cucumbers/atom_init()
	. = ..()
	for(var/i = 1 to rand(1, 7))
		new /obj/item/weapon/reagent_containers/food/snacks/grown/cucumber/pickled(src)
		cucumbers +=1
	verbs += /obj/item/weapon/storage/pickled_cucumbers/proc/gulp_whole
	verbs += /obj/item/weapon/storage/pickled_cucumbers/proc/lid
...
/obj/item/weapon/storage/pickled_cucumbers/update_icon()
	cut_overlay("cucumbers_[cucumbers + 1]")
	add_overlay("cucumbers_[cucumbers]")

	if(reagents.total_volume == 0)
		icon_state = "empty_jar"
	else
		icon_state = "brine_jar"

/obj/item/weapon/storage/pickled_cucumbers/examine(mob/user)
	..()
	if(src in user)
		if(!reagents || reagents.total_volume==0)
			to_chat(user, "<span class='notice'>\The [src] is empty!</span>")
	...
	if(src in view(1, user))
		if(contents.len <= 0)
			to_chat(user, "There are no cucumbers left in the jar.")
		else if(contents.len == 1)
			to_chat(user, "There is one cucumber left in the jar.")
		else
			to_chat(user, "There are [src.contents.len] cucumbers in the jar.")

/obj/item/weapon/storage/pickled_cucumbers/open(mob/user)
	if (!is_open_container())
		to_chat(user, "<span class='notice'>You need to open [src]!</span>")
		return
	..()

/obj/item/weapon/storage/pickled_cucumbers/handle_item_insertion()
	..()
	cucumbers += 1
	update_icon()

/obj/item/weapon/storage/pickled_cucumbers/remove_from_storage()
	..()
	cucumbers -= 1
	update_icon()

/obj/item/weapon/storage/pickled_cucumbers/proc/gulp_whole()
	set category = "Object"
	set name = "Gulp Down"
	set src in view(1)
...
/obj/item/weapon/storage/pickled_cucumbers/attack(mob/living/M, mob/user, def_zone)
	if (!is_open_container())
		return 0
...
/obj/item/weapon/storage/pickled_cucumbers/afterattack(atom/target, mob/user, proximity, params)
	if(!proximity)
		return
...
/obj/item/weapon/storage/pickled_cucumbers/proc/lid()
	set category = "Object"
...
/obj/item/weapon/storage/pickled_cucumbers/proc/reagentlist(obj/item/weapon/reagent_containers/snack) //Attack logs for regents in pills
	var/data
